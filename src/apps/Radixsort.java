...
public class Radixsort {
	...
	throws IOException {
		...
		Node<String> firstNode = masterListRear;
		while(sc.hasNextLine()) {
			Node<String> temp = new Node<String>(sc.nextLine(), firstNode);
	...
	public void scatter(int pass) {
		 ...
		 do {
			 ...
			 if (ptr.data.length() >= (pass + 1)) {
				 i = Character.digit(ptr.data.charAt(ptr.data.length()-1-pass), radix);
			 }
			 if (buckets[i] == null) {
				 buckets[i] = ptr;
				 ptr=ptr.next;
				...
				Node<String> front = buckets[i].next;
				Node<String> n = ptr;
				ptr = ptr.next;
				n.next = front;
				buckets[i].next = n;
	...
	public void gather() {
		...
		for(int j = 0; j < buckets.length; j++) {
			if(buckets[j] != null) {
				Node<String> temp = buckets[j].next;
				Node<String> z = temp;
 				do {
